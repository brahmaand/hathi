...
internal class CFile
{
    ...
    public string FileName
    {
        ...
        set
        {
            ...
            {
                File.Move(m_CompleteName,Path.GetDirectoryName(m_CompleteName)+Path.DirectorySeparatorChar+value);
                m_FileName=value;
                m_FileDetails.FileName=m_FileName;
                m_CompleteName=Path.GetDirectoryName(m_CompleteName)+Path.DirectorySeparatorChar+value;
    ...
    {
        get {
            return m_DownPriority;
        }
        set {
            m_DownPriority=value;
        }
    }
                 ...
                 byte permisos, ArrayList in_HashSet, string in_strComment, byte in_rating)
    {
        ...
        m_CompleteName=string.Concat(m_FileFolder, "\\" , m_FileName);
    ...
    public CFile(byte[] in_FileHash, string in_realfilename, uint in_FileSize)
    {
        ...
        while (File.Exists(in_fileFolder + "\\" + cont.ToString("000") + ".part"))
        {
            cont++;
        }
        ...
        m_MyComment="";
        m_CompleteName      = string.Concat(m_FileFolder, "\\" , m_PartFileName);
    ...
    private void m_AddGap(uint start, uint end)
    {
        ...
        if (end==m_FileSize-2) end=m_FileSize-1; //for compatibility with old Hathi format::: remove for next releases
    ...
    private void m_ThreadCompleteFile()
    {
        ...
        else
        {
            if (m_CloseFileWrite())
            {
                ...
                string newCompleteName=string.Concat(m_FileFolder, "\\" , filenametodisk);
                int i=0;
                while (File.Exists(newCompleteName))
                {
                    ...
                    newCompleteName=string.Concat(m_FileFolder, "\\" , filenametodisk);
                    i++;
                }
    ...
    public void CancelFile()
    {
        ...
        try
        {
            File.Delete(m_CompleteNameMet);
            File.Delete(m_CompleteNameMet + ".backup");
            File.Delete(Path.GetDirectoryName(m_CompleteNameMet)+"\\"+m_PartFileName);
        }
        catch {}
    }
