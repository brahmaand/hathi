...
namespace Hathi.Client
{
	...
	public class FormSearch : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.tabSearchs = new TabControl();
		...
		private void buttonExtend_Click(object sender, System.EventArgs e)
		{
			if (buttonExtend.Text == HathiForm.Globalization["LBL_CANCEL"])
			{
				...
				if (tabSearchs.SelectedIndex >= 0) m_OnEndSearch((int)tabSearchs.ActiveTabPage.Tag);
				if (tabSearchs.SelectedIndex >= 0) krnGateway.CancelSearch((int)tabSearchs.ActiveTabPage.Tag);
			}
			else
			{
				//if (tabSearchs.SelectedIndex >= 0) krnGateway.ExtendSearch((int)tabSearchs.TabPages[tabSearchs.SelectedIndex].Tag);
				if (tabSearchs.SelectedIndex >= 0) krnGateway.ExtendSearch((int)tabSearchs.ActiveTabPage.Tag);
			}
		}
		protected void OnClosePressed(object sender, EventArgs e)
		{
			//if (tabSearchs.SelectedIndex < 0) return;
			if (tabSearchs.ActiveTabPage == null) return;
			//if (tabSearchs.SelectedIndex == tabSearchs.TabPages.Count - 1)
			if (tabSearchs.SelectedIndex == tabSearchs.Count - 1)
				//m_OnEndSearch((int)tabSearchs.TabPages[tabSearchs.SelectedIndex].Tag);
				m_OnEndSearch((int)tabSearchs.GetTabPageAt(tabSearchs.SelectedIndex).Tag);
			int tabIndexToRemove = (int)tabSearchs.GetTabPageAt(tabSearchs.SelectedIndex).Tag;
			krnGateway.DeleteSearch((int)tabSearchs.GetTabPageAt(tabSearchs.SelectedIndex).Tag);
