...
namespace Hathi.Client
{
	...
	public class FormFriends : System.Windows.Forms.Form
	{
		...
		private void OnAddFriend(object sender, System.EventArgs e)
		{
			...
			if (tabMessages.Count <= 0) return;
			//if (tabMessages.SelectedTab == null) return;
			if (tabMessages.ActiveTabPage == null) return;
			//InterfaceClient source = (InterfaceClient)tabMessages.SelectedTab.Tag;
			InterfaceClient source = (InterfaceClient)tabMessages.ActiveTabPage.Tag;
		...
		private void m_ProcessMessage()
		{
			...
			foreach (TabPage page in tabMessages.TabPages)
			{
			...
			if (messagePage == null)
			{
				...
				messagePage = new TabPage(m_LastClient.Name, textBoxMsg);
				messagePage.Tag = m_LastClient;
				//tabMessages.TabPages.Add(messagePage);
				tabMessages.Add(messagePage);
		...
		private void InitializeComponent()
		{
			...
			this.tabMessages = new TabControl();
		...
		private void buttonSend_Click(object sender, System.EventArgs e)
		{
			...
			TabPage messagePage = tabMessages.ActiveTabPage;
			if (messagePage == null)
				return;
